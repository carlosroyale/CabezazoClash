...
function startBasicGame({canvas, ctx: ctxParam, scoreEl: scoreElParam, timerEl: timerElParam, onExit, bot = false, playerNames}) {
    ...
    // Resetear estados de finalización
...
function updateTimer() {
    if (gameTimerEl) {
        // Evitar el frame residual que redondea a 1 al acabar el partido online
        if (matchFinishedExternally) gameTimerEl.textContent = "0";
        else gameTimerEl.textContent = `${Math.max(0, Math.ceil(gameTime))}`;
    }
}
...
window.Game = {
    ...
    isFinished: () => matchFinishedExternally,
    forceRedraw
};